...
def generate_candidates_probs(candidates, word, channel_prob, bigram_count, term_count, line, j):
    ...
    for candi in candidates:
        ...
        if j > 0:
            ...
            prob += calculate_smoothed_probability(bigram_count, term_count, len(term_count.keys()), forward_word, line[j - 1])
        if j + 1 < len(line):
            backward_word = candi + " " + line[j + 1]  # 考虑后一个单词，出现playing football的概率
            prob += calculate_smoothed_probability(bigram_count, term_count, len(term_count.keys()), backward_word, candi)
        probs.append(prob)
    return probs
...
class spell_correction():
    def __init__(self):
        ...
        try:
            ...
            print("first time running, download reuters corpus, it will take some time.")
    ...
    def correction(self, line):
        ...
        for j, word in enumerate(line):
            ...
            if word not in self.vocab:
                ...
                probs = generate_candidates_probs(candidates, word, self.channel_prob, self.bigram_count, self.term_count, line, j)
                ...
                for word_candidate in word_candidates:
                    ...
                    for m in range(len(sentence) - 1):  # 采用链式法则计算概率，同时还有一项将当前词错写成候选词的概率
                        ...
                        sentence_prob += (calculate_smoothed_probability(
                            self.bigram_count, self.term_count, self.V, (sentence_word, sentence_word_next), sentence_word))
                    ...
                    for correct_char, incorrect_char in pairs.items():
                        if correct_char in self.channel_prob and incorrect_char in self.channel_prob[correct_char]:
                            sentence_prob += np.log(self.channel_prob[correct_char][incorrect_char])
